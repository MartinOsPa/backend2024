...
const pokemonSeeder = async ()=> {
    //const pokemons = await fetch ('https://pokeapi.co/api/v2/pokemon/?offset=0&limit=151') .then (res => res.json()); //de JSON a algo que pueda entender JS
    const {results} = await fetch ('https://pokeapi.co/api/v2/pokemon/?offset=0&limit=151') .then (res => res.json());

    //console.log(pokemons);//verificar que todo este OK 
    //console.log(results);
